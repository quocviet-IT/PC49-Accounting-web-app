-- 0065_a_lot_arrives_with_its_bags.sql
-- Một lô phân kim đi vào hệ thống cùng những túi đã đi trong nó.
--
-- Bộ nạp cũ chỉ dựng được đầu lô — mã, nhà máy, ngày, spot — và một lô không
-- có túi nào thì không có gì để tính: bảng xanh lá đọc từ từng túi. Sheet 3.2
-- vốn đã viết mỗi túi một dòng, nên nguồn REFINING_LOT giờ nhận đúng như thế:
-- một dòng là một túi, các dòng cùng mã lô dựng chung một lô.
--
-- Chỗ tế nhị là chân chuyển kho. Trigger `refining_lot_send_legs` (0056) sinh
-- một TRANSFER_OUT cho mỗi túi khi lô CHUYỂN sang trạng thái đã gửi. Nhưng ba
-- lô của năm 2026 đã có chân rồi: chính là các dòng Transfer kế toán ghi trong
-- sheet giao dịch. Nên lô được dựng THẲNG ở trạng thái cuối của nó bằng một câu
-- INSERT; trigger chỉ chạy trên UPDATE nên không sinh bộ chân thứ hai, và sổ
-- giữ đúng một bộ — bộ có thật.
...
INSERT INTO pc49.schema_migrations (version) VALUES ('0065_a_lot_arrives_with_its_bags')
