...
- **Quyết định chiều 14-09** (anh Việt chọn, khi hai tệp vẫn chưa về):
  ...
  - Đường nạp là đường của màn hình Nạp dữ liệu. `scripts/load-2026.mjs` (qua `scripts/lib/load-gold-batch.mjs`) đi y hệt đường đó, chỉ khác là stage cả tệp trong một câu lệnh; có `--dry-run` để hoàn tác, và chạy thử trên PGlite ra đúng con số của lần stage từng dòng. Màn hình `stage_import_row` từng dòng, nút "Ghi phần nhận được, bỏ lại dòng lỗi" là `commit_import_batch(batch, true)`, và không ghi sổ. Chữ "Ghi vào sổ" trên màn hình đó nghĩa là đưa giao dịch vào hệ thống, không phải sinh bút toán.
  - **Đã chạy trên cơ sở dữ liệu thật chiều 14-09**, sau khi anh Việt cấp quyền cho ba lệnh `npm run migrate`, `npm run demo:clear` và `npm run load:2026` (trong `.claude/settings.local.json`, không vào git). Thứ tự chạy:
    1. `migrate` áp 0062–0067.
    2. `load:2026 --dry-run` khớp từng con số với lần chạy thử trên PGlite, rồi hoàn tác.
    3. `demo:clear` xoá dữ liệu demo.
    4. `load:2026` nạp **1.061 giao dịch, chưa ghi sổ**, thành sáu lô `01-gold-txn-2026-*.csv` trên màn hình Nạp dữ liệu.

    Giao dịch mua ML ngày 29/08 vẫn còn. Trên màn hình đó, lô không có dòng bị trả hiện chữ "Đã ghi sổ": nghĩa là đã nạp, không phải đã sinh bút toán.
